...
export async function recordMemberActivity(db: Firestore, uid: string, type: MemberActivityType, targetId: string, title = 'İçerik', href = '/bilgi-merkezi') {
  ...
  await setDoc(doc(db, 'memberActivities', `${uid}_${type}_${safeTarget}`), { uid, type, targetId: safeTarget, title: title.slice(0, 140), href: href.slice(0, 180), points: activityPoints[type], createdAt: serverTimestamp() });
}
