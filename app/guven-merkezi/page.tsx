...
export default function TrustCenterPage() {
  return <main className="trust-v1-page">
    ...
    <section id="kontrol" className="trust-v1-section trust-v1-check-section"><div className="trust-v1-shell"><div className="trust-v1-section-head"><div><p className="trust-v1-eyebrow">01 / İŞLEM ÖNCESİ</p><h2>Dört kısa kontrolü tamamlayın.</h2></div><span>Sonraki adıma geçmeden önce resmî kanal, hesap, net tutar ve erişim sınırını doğrulayın.</span></div><SkyTrustCheck /></div></section>
    ...
    <section className="trust-v1-section trust-v1-final-section"><div className="trust-v1-shell"><div className="trust-v1-section-head"><div><p className="trust-v1-eyebrow">04 / DOĞRU YÖNLENDİRME</p><h2>Kontrolden sonra doğru yere geçin.</h2></div><Link href="/iletisim" className="trust-v1-inline-link">İletişim merkezini aç →</Link></div><div className="trust-v1-final-grid"><div className="trust-v1-links"><Link href="/bilgi-merkezi/sky-bozum-iletisim-rehberi"><b>Resmî iletişimi doğrula</b><span>Doğru kanal ve paylaşım sınırlarını kontrol et →</span></Link><Link href="/bilgi-merkezi/sorun-cozme"><b>Sorun çözme merkezi</b><span>Yaşadığınız duruma uygun adımları bulun →</span></Link><Link href="/referanslar"><b>Referansları incele</b><span>Kaynaklı kullanıcı kayıtlarını görüntüle →</span></Link></div><div className="trust-v1-faq-panel"><div><p className="trust-v1-eyebrow">KISA CEVAPLAR</p><strong>En sık sorulan güvenlik soruları</strong></div><div className="trust-v1-faqs">{faqs.map(([question, answer]) => <details key={question}><summary>{question}<b>+</b></summary><p>{answer}</p></details>)}</div></div></div></div></section>
    <TrustQuickDock />
  </main>;
}
