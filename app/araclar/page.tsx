  ...
  return <main className="tools-center tc2-page">
    ...
    <section className="tc2-section tc2-lower-section">
      <div className="tc2-shell tc2-lower-grid">
        ...
        <div className="tc2-seo-block"><div className="tc2-section-heading tc2-section-heading--compact"><div><p className="tc2-eyebrow">03 / SEO VE METİN</p><h2>İçeriğini güçlendir.</h2></div></div><div className="tc2-seo-visual"><Image src="/images/araclar/editorial-v6/yazili-teyit.webp" alt="Metin ve içerik çalışma alanı" fill sizes="(max-width: 900px) 100vw, 30vw" /></div><div className="tc2-seo-list">{seoTools.map(([label, description, tab]) => <Link href={`/araclar/seo?tab=${tab}`} key={tab}><span>{label}</span><small>{description}</small><b aria-hidden="true">→</b></Link>)}</div></div>
      </div>
    </section>
  </main>;
}
