...
export default async function TroubleshootingDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#090b10] text-white">
    ...
    <header className="border-b border-white/8 py-14 sm:py-20"><div className="content-shell">
      <nav aria-label="Sayfa yolu" className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500"><Link href="/">Ana Sayfa</Link><span>/</span><Link href="/bilgi-merkezi">Bilgi Merkezi</Link><span>/</span><Link href="/bilgi-merkezi/sorun-cozme">Sorun Çözme</Link></nav>
    ...
    <div className="content-shell grid gap-8 py-12 lg:grid-cols-[minmax(0,1fr)_340px]">
      <article className="space-y-8">
        ...
        <section className="premium-card p-6 sm:p-8"><p className="text-xs font-extrabold uppercase tracking-[.16em] text-emerald-300">Adım adım kontrol</p><div className="mt-6 space-y-5">{guide.checks.map((check, index) => <div key={check.title} className="rounded-2xl border border-white/8 bg-white/[.025] p-5"><div className="flex gap-4"><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-emerald-400/10 text-sm font-black text-emerald-300">{index + 1}</span><div><h2 className="text-lg font-black">{check.title}</h2><p className="mt-2 text-sm leading-7 text-slate-400">{check.text}</p></div></div></div>)}</div></section>
      ...
      <aside className="space-y-4 lg:sticky lg:top-28 lg:self-start">
        {service && <div className="premium-card p-6"><p className="text-xs font-extrabold uppercase tracking-[.15em] text-rose-400">İlgili hizmet</p><h2 className="mt-3 text-xl font-black">{service.shortName}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{service.summary}</p><Link href={`/hizmetler/${service.slug}`} className="btn-secondary focus-ring mt-5 w-full">Hizmet sayfasını aç</Link></div>}
        <div className="premium-card p-6"><p className="text-xs font-extrabold uppercase tracking-[.15em] text-slate-500">Devam edin</p><div className="mt-4 space-y-3"><Link href="/bilgi-merkezi/sorun-cozme" className="focus-ring block rounded-xl border border-white/8 p-4 text-sm font-black">Tüm sorun rehberleri →</Link><Link href="/iletisim" className="focus-ring block rounded-xl border border-white/8 p-4 text-sm font-black">İletişim seçenekleri →</Link></div></div>
      </aside>
    </div>
    {relatedArticles.length > 0 && <section className="border-t border-white/8 bg-[#0d1016] py-14"><div className="content-shell"><p className="text-xs font-extrabold uppercase tracking-[.18em] text-rose-400">İlgili rehberler</p><div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{relatedArticles.map((article) => article && <Link key={article.slug} href={`/bilgi-merkezi/${article.slug}`} className="premium-card focus-ring p-5"><h2 className="text-lg font-black leading-7">{article.title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{article.excerpt}</p></Link>)}</div></div></section>}
  </main>;
}
