...
export default function TroubleshootingPage() {
  ...
  return <main className="min-h-screen bg-[#090b10] text-white">
    ...
    <header className="border-b border-white/8 py-16 sm:py-20"><div className="content-shell">
      ...
      <div className="mt-8 flex flex-wrap gap-3">{categories.map(category=><a key={category} href={`#${category.toLocaleLowerCase('tr-TR').replace(/\s+/g,'-')}`} className="rounded-full border border-white/10 bg-white/[.04] px-4 py-2 text-sm font-bold text-slate-300 hover:border-rose-400/40 hover:text-white">{category}</a>)}</div>
    </div></header>
