...
export function FaqModule({
  ...
  return <section className={`relative overflow-x-clip bg-[#faf8f9] text-slate-950 dark:bg-[#090a0f] dark:text-white ${className}`}>
    {schema && hasItems && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeJsonLd(schema) }}/>} 
    <div aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 h-[430px] bg-[radial-gradient(circle_at_18%_0%,rgba(231,45,94,0.08),transparent_58%)] dark:bg-[radial-gradient(circle_at_18%_0%,rgba(231,45,94,0.15),transparent_58%),radial-gradient(circle_at_86%_8%,rgba(242,160,83,0.08),transparent_44%)]"/>
    <div className="relative mx-auto w-full max-w-[1320px] px-4 pb-14 pt-9 sm:px-6 sm:pb-18 sm:pt-14 lg:px-8 lg:pb-22 lg:pt-[68px] xl:px-10">
      ...
      <div className="mt-9 lg:mt-[52px]">
        ...
        {isSearching ? <div className="mx-auto max-w-[900px]"><div className="mb-6 flex flex-col gap-4 border-b border-slate-200/80 pb-5 sm:flex-row sm:items-end sm:justify-between dark:border-white/10"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-rose-700 dark:text-rose-300">Arama sonuçları</p><h2 className="mt-2 text-[24px] font-bold tracking-[-0.035em] text-slate-950 sm:text-[28px] dark:text-white">“{effectiveQuery}” için {totalResultCount} sonuç</h2>{totalResultCount > SEARCH_RESULT_RENDER_LIMIT && <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-[#a79ca4]">En alakalı ilk {SEARCH_RESULT_RENDER_LIMIT} sonuç gösteriliyor. Aramanızı daraltarak daha kesin sonuçlara ulaşabilirsiniz.</p>}</div><button type="button" onClick={() => setQuery("")} className="self-start rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-rose-300 hover:text-rose-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 sm:self-auto dark:border-white/10 dark:bg-[#101117] dark:text-[#e8e1e5]">Aramayı temizle</button></div><SearchResults items={results} onSelect={selectSearchResult} supportLinks={supportLinks}/></div> : <div className="grid items-start gap-6 lg:grid-cols-[248px_minmax(0,1fr)] xl:gap-9 2xl:grid-cols-[252px_minmax(0,1fr)]">
          <FaqCategoryRail active={activeCategory} categories={categories} onSelect={selectCategory} stickyTop={stickyTop}/>
          <section aria-labelledby={contentHeadingId} className="min-w-0"><div className="mb-6 flex flex-col gap-3 border-b border-slate-200/75 pb-5 sm:flex-row sm:items-end sm:justify-between dark:border-white/10"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-rose-700 dark:text-rose-300">Seçili kategori</p><h2 id={contentHeadingId} tabIndex={-1} className="mt-2 text-[23px] font-bold leading-[1.15] tracking-[-0.035em] text-slate-950 outline-none sm:text-[27px] dark:text-white">{activeCategory}</h2><p className="mt-2 max-w-2xl text-[14px] leading-[1.65] text-slate-500 dark:text-[#a79ca4]">{categoryDescription}</p></div><span className="hidden shrink-0 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-500 sm:inline-flex dark:border-white/10 dark:bg-[#101117] dark:text-[#a79ca4]">{categoryItems.length} soru</span></div><FaqAccordion items={categoryItems} openId={openId} onToggle={toggle} instanceId={instanceId}/><FaqTrustStrip items={trustItems}/></section>
        </div>}
        </>}
      </div>
    </div>
