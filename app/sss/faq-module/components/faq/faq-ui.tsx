...
function CategoryGlyph({ active }: { active: boolean }) {
  return <span aria-hidden="true" className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-[9px] border transition ${active ? "border-rose-200 bg-white text-rose-700 shadow-[0_2px_8px_rgba(225,29,72,.12)] dark:border-rose-400/25 dark:bg-[#151017] dark:text-rose-300" : "border-slate-200/90 bg-white text-slate-400 dark:border-white/10 dark:bg-[#101117] dark:text-[#81757d]"}`}>
...
export function FaqHeader({ title, description, query, onQueryChange, resultCount, isSearching, searchPlaceholder }: { title: string; description: string; query: string; onQueryChange: (value: string) => void; resultCount: number; isSearching: boolean; searchPlaceholder: string }) {
  ...
  return <header className="mx-auto max-w-[860px] text-center">
    <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.22em] text-amber-700 sm:text-[10.5px] dark:text-[#f2c98a]">Sky Bozum Destek Merkezi</p>
    ...
    <p className="mx-auto mt-3.5 max-w-[680px] text-[15px] leading-[1.7] text-slate-600 sm:text-[16px] sm:leading-[1.72] dark:text-[#c9bfc5]">{description}</p>
...
function CategoryList({ active, categories, onSelect, query = "", onQueryChange }: { active: FaqCategory; categories: readonly FaqCategory[]; onSelect: (category: FaqCategory) => void; query?: string; onQueryChange?: (value: string) => void }) {
  ...
  return <div>
    {onQueryChange && <div className="relative mb-5"><span className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400"><SearchIcon className="h-[17px] w-[17px]"/></span><input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="Kategori ara…" className="h-11 w-full rounded-xl border border-slate-200/90 bg-slate-50/80 pl-10 pr-3 text-[13px] outline-none transition focus:border-rose-400 focus:bg-white focus:ring-4 focus:ring-rose-100 dark:border-white/10 dark:bg-white/[.04] dark:text-white dark:focus:ring-rose-500/10"/></div>}
    {!visibleGroupCount && <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-center text-xs leading-5 text-slate-500 dark:border-white/10 dark:bg-[#101117] dark:text-[#a79ca4]">Bu adla eşleşen kategori bulunamadı.</div>}
    <nav aria-label="SSS kategorileri" className="space-y-6">
      {groups.map((group) => {
        ...
        return <section key={group.label}><p className="mb-2.5 px-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">{group.label}</p><div className="space-y-1">{visible.map((category) => {
          ...
          return <button key={category} type="button" onClick={() => onSelect(category)} aria-current={isActive ? "true" : undefined} className={`group relative flex min-h-[46px] w-full items-center gap-3 rounded-xl px-2.5 py-1.5 text-left text-[13.5px] font-medium transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 ${isActive ? "bg-rose-50/75 text-rose-800 dark:bg-rose-400/[.09] dark:text-rose-200" : "text-slate-600 hover:bg-slate-50/80 hover:text-slate-950 dark:text-[#c9bfc5] dark:hover:bg-white/[.035] dark:hover:text-white"}`}><span aria-hidden="true" className={`absolute inset-y-2.5 left-0 w-[2px] rounded-full transition ${isActive ? "bg-rose-600" : "bg-transparent"}`}/><CategoryGlyph active={isActive}/><span className="min-w-0 leading-[1.35]">{category}</span></button>;
        })}</div></section>;
      })}
    </nav>
  </div>;
...
export function FaqCategoryRail({ active, categories, onSelect, stickyTop }: { active: FaqCategory; categories: readonly FaqCategory[]; onSelect: (category: FaqCategory) => void; stickyTop: string | number }) {
  ...
  return <aside className="hidden lg:block"><div className="max-h-[calc(100vh-7rem)] overflow-y-auto rounded-[18px] border border-slate-200/80 bg-white/95 p-3 shadow-[0_12px_34px_rgba(15,23,42,0.05)] backdrop-blur dark:border-white/10 dark:bg-[#101117]" style={{ position: "sticky", top: stickyTop }}><div className="px-2 pb-3 pt-1"><p className="text-[15px] font-bold tracking-[-0.01em] text-slate-950 dark:text-white">Kategoriler</p><p className="mt-1.5 text-xs leading-5 text-slate-500 dark:text-[#a79ca4]">İhtiyacınız olan başlığı seçin.</p></div><CategoryList active={active} categories={categories} onSelect={onSelect} query={query} onQueryChange={setQuery}/></div></aside>;
}
...
export function FaqCategoryDrawer({ active, categories, onSelect }: { active: FaqCategory; categories: readonly FaqCategory[]; onSelect: (category: FaqCategory) => void }) {
  ...
  return <div className="lg:hidden">
    <button ref={triggerRef} type="button" aria-haspopup="dialog" aria-expanded={open} onClick={() => { focusTaskRef.current?.cancel(); closeReasonRef.current = "dismiss"; setOpen(true); }} className="flex min-h-[54px] w-full items-center justify-between gap-4 rounded-2xl border border-slate-200/90 bg-white px-4 text-left shadow-[0_8px_24px_rgba(15,23,42,.05)] transition hover:border-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 dark:border-white/10 dark:bg-white/[.03]"><span className="flex min-w-0 items-center gap-3 text-sm font-semibold text-slate-950 dark:text-white"><span className="text-rose-700 dark:text-rose-300"><MenuIcon/></span><span className="truncate">{active}</span></span><span className="text-xs font-semibold text-rose-700 dark:text-rose-300">Değiştir</span></button>
    {open && <div className="fixed inset-0 z-50"><button type="button" aria-label="Kategori panelini kapat" onClick={dismiss} className="absolute inset-0 bg-slate-950/45 backdrop-blur-[2px]"/><div ref={panelRef} role="dialog" aria-modal="true" aria-label="Kategori seçici" tabIndex={-1} className="absolute inset-y-0 right-0 flex w-[min(88vw,360px)] flex-col border-l border-slate-200 bg-white shadow-[-24px_0_70px_rgba(15,23,42,0.2)] outline-none dark:border-white/10 dark:bg-[#090a0f]"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-5 dark:border-white/10"><div><h2 className="text-lg font-bold tracking-[-0.02em] text-slate-950 dark:text-white">Kategoriler</h2><p className="mt-1 text-xs text-slate-500">Başlıklar arasında hızlı geçiş yapın.</p></div><button type="button" onClick={dismiss} aria-label="Kapat" className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-500 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 dark:border-white/10 dark:hover:bg-white/[.05]"><CloseIcon/></button></div><div className="flex-1 overflow-y-auto p-4"><CategoryList active={active} categories={categories} onSelect={select} query={query} onQueryChange={setQuery}/></div></div></div>}
  </div>;
}
...
function SupportActions({ links }: { links?: { whatsapp?: string; liveSupport?: string; contact?: string } }) {
  ...
  return <div className="mt-6 flex flex-wrap justify-center gap-3">{actions.map((action, index) => <a key={action.label} href={action.href} className={`${index === 0 ? "border-rose-600 bg-gradient-to-r from-rose-600 to-orange-500 text-white hover:from-rose-500 hover:to-orange-400" : "border-slate-200 bg-white text-slate-700 hover:border-rose-300 hover:text-rose-700 dark:border-white/10 dark:bg-white/[.03] dark:text-[#e8e1e5]"} inline-flex min-h-11 items-center justify-center rounded-xl border px-4 py-2.5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500`}>{action.label}</a>)}</div>;
}
...
export function FaqEmptyState({ message, supportLinks }: { message: string; supportLinks?: { whatsapp?: string; liveSupport?: string; contact?: string } }) {
  return <div role="status" aria-live="polite" className="mx-auto max-w-[760px] rounded-[20px] border border-dashed border-slate-300 bg-slate-50/75 px-6 py-14 text-center sm:px-10 sm:py-16 dark:border-white/15 dark:bg-white/[.025]">
    <span aria-hidden="true" className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-rose-700 ring-1 ring-inset ring-slate-200 shadow-[0_8px_24px_rgba(15,23,42,.06)] dark:bg-white/[.04] dark:text-rose-300 dark:ring-white/10"><svg viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-current" strokeWidth="1.7"><path d="M5.5 6.5h13v11h-13z"/><path d="M8.5 10h7M8.5 13.5h4.5"/></svg></span>
    <h2 className="mt-5 text-[22px] font-bold tracking-[-0.03em] text-slate-950 sm:text-[24px] dark:text-white">{message}</h2>
    <p className="mx-auto mt-3 max-w-xl text-[14px] leading-6 text-slate-500 sm:text-[15px] dark:text-[#a79ca4]">Aradığınız konu şu anda bu bölümde bulunmuyor. Güncel bilgi için destek ekibimize ulaşabilirsiniz.</p>
...
export function FaqAccordion({ items, openId, onToggle, instanceId }: { items: readonly FaqItem[]; openId?: string; onToggle: (id: string) => void; instanceId: string; query?: string }) {
  return <div className="overflow-hidden rounded-[16px] border border-slate-200/90 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.04)] dark:border-white/10 dark:bg-white/[.018]">{items.map((item, index) => {
    ...
    return <article key={item.id} id={`${instanceId}-faq-${item.id}`} className={`${index ? "border-t border-slate-200/75 dark:border-white/[.075]" : ""} transition-colors duration-300 ${open ? "bg-rose-50/30 dark:bg-rose-400/[.035]" : "bg-transparent"}`}><h3><button id={buttonId} type="button" aria-expanded={open} aria-controls={panelId} onClick={() => onToggle(item.id)} className="group flex min-h-[62px] w-full items-center justify-between gap-4 px-4 py-[15px] text-left text-[14px] font-semibold leading-[1.5] text-slate-950 outline-none transition-colors duration-200 hover:bg-slate-50/65 hover:text-rose-800 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-rose-500 sm:gap-5 sm:px-6 sm:text-[15.5px] sm:leading-[1.48] dark:text-white dark:hover:bg-white/[.025] dark:hover:text-rose-300"><span className="min-w-0">{item.question}</span><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors duration-200 ${open ? "bg-rose-100/90 text-rose-700 dark:bg-rose-400/10 dark:text-rose-300" : "bg-slate-50 text-slate-400 group-hover:bg-white group-hover:text-slate-600 dark:bg-white/[.04] dark:group-hover:text-[#c9bfc5]"}`}><Chevron open={open}/></span></button></h3>{open && <div id={panelId} role="region" aria-labelledby={buttonId} className="px-4 pb-5 text-[14px] leading-[1.74] text-slate-600 sm:px-6 sm:pb-6 sm:text-[15px] sm:leading-[1.72] dark:text-[#c9bfc5]"><div className="max-w-[800px] border-l-2 border-rose-200/90 pl-4 sm:pl-5 dark:border-rose-400/25"><p>{item.shortAnswer}</p>{item.answer !== item.shortAnswer && <p className="mt-3.5">{item.answer}</p>}{relatedArticleHref(item.category) ? <a href={relatedArticleHref(item.category)} className="mt-4 inline-flex font-semibold text-rose-700 hover:underline dark:text-rose-300">Ayrıntılı rehberi incele →</a> : null}</div></div>}</article>;
  })}</div>;
}
...
export function SearchResults({ items, onSelect, supportLinks }: { items: readonly FaqItem[]; onSelect: (item: FaqItem) => void; supportLinks?: { whatsapp?: string; liveSupport?: string; contact?: string } }) {
  ...
  return <div className="overflow-hidden rounded-[18px] border border-slate-200/90 bg-white shadow-[0_14px_40px_rgba(15,23,42,.045)] dark:border-white/10 dark:bg-[#101117]">{items.map((item, index) => <button key={item.id} type="button" onClick={() => onSelect(item)} className={`group block w-full p-5 text-left transition hover:bg-slate-50/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-rose-500 sm:p-6 dark:hover:bg-white/[.025] ${index ? "border-t border-slate-200/80 dark:border-white/[.08]" : ""}`}><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-rose-700 dark:text-rose-300">{item.category}</span><span className="mt-2 block text-[15px] font-semibold leading-6 text-slate-950 transition group-hover:text-rose-800 dark:text-white dark:group-hover:text-rose-300">{item.question}</span><span className="mt-2 line-clamp-2 block text-sm leading-6 text-slate-500 dark:text-[#a79ca4]">{item.shortAnswer}</span></button>)}</div>;
}
