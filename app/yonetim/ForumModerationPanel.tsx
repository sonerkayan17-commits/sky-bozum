...
import { addDoc, collection, doc, getDocs, onSnapshot, query, serverTimestamp, updateDoc, type Firestore } from 'firebase/firestore';
...
type ContentReport = { id: string; targetId: string; reason: string; status: string; createdAt: Date | null };
...
export default function ForumModerationPanel({ db, actorId }: { db: Firestore | null; actorId: string }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, 'contentReports'), (snapshot) => {
      setReports(snapshot.docs.map((entry) => { const data = entry.data(); return { id: entry.id, targetId: String(data.targetId || ''), reason: String(data.reason || ''), status: String(data.status || 'open'), createdAt: data.createdAt?.toDate?.() ?? null }; }).sort((a, b) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0)));
  ...
  async function startEditing(post: ForumPost) {
    ...
    setNotice('');
    if (db) {
      const snapshot = await getDocs(query(collection(db, 'forumPostRevisions')));
      setRevisions(snapshot.docs.filter((entry) => entry.data().postId === post.id).map((entry) => ({ id: entry.id, title: String(entry.data().title || ''), body: String(entry.data().body || ''), createdAt: entry.data().createdAt?.toDate?.() ?? null })).sort((a, b) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0)));
    }
  }
  ...
  return <section className="admin-section">
    ...
    {reports.filter((report) => report.status === 'open').length > 0 && <section className="admin-comments"><h3>Açık içerik raporları ({reports.filter((report) => report.status === 'open').length})</h3>{reports.filter((report) => report.status === 'open').map((report) => <article key={report.id}><div><strong>Konu: {report.targetId}</strong><p>{report.reason}</p><small>{report.createdAt?.toLocaleDateString('tr-TR') || 'Yeni rapor'}</small></div><button className="admin-secondary compact" onClick={() => void resolveReport(report)}>Raporu kapat</button></article>)}</section>}
