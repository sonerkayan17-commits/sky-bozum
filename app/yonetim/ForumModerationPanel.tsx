...
export default function ForumModerationPanel({ db, actorId }: { db: Firestore | null; actorId: string }) {
  ...
  return <section className="admin-section">
    ...
    {reports.filter((report) => report.status === 'open').length > 0 && <section className="admin-comments"><h3>Açık içerik raporları ({reports.filter((report) => report.status === 'open').length})</h3>{reports.filter((report) => report.status === 'open').map((report) => <article key={report.id}><div><strong>{report.targetType === 'comment' ? 'Yorum' : 'Konu'}: {reportTarget(report)}</strong><p>{report.reason}</p><small>Kaynak ID: {report.targetId} · {report.createdAt?.toLocaleDateString('tr-TR') || 'Yeni rapor'}</small></div><button className="admin-secondary compact" onClick={() => void resolveReport(report)}>Raporu kapat</button></article>)}</section>}
