...
import { addDoc, collection, deleteDoc, doc, getDocs, onSnapshot, query, serverTimestamp, updateDoc, where, type Firestore } from 'firebase/firestore';
...
export default function ForumModerationPanel({ db, actorId }: { db: Firestore | null; actorId: string }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, 'comments'), (snapshot) => {
      setComments(snapshot.docs.map((entry) => {
        const data = entry.data();
        return { id: entry.id, service: String(data.service || ''), author: String(data.author || 'Üye'), message: String(data.message || '') };
      }));
  ...
  function reportTarget(report: ContentReport) {
    if (report.targetType === 'comment') {
      ...
      return comment ? `${comment.author}: ${comment.message.slice(0, 180)}` : `Yorum kaydı bulunamadı (${report.targetId})`;
    }
  ...
  async function removeReportedComment(report: ContentReport) {
    ...
    try {
      ...
      await updateDoc(doc(db, 'contentReports', report.id), { status: 'resolved', resolvedBy: actorId, resolvedAt: serverTimestamp() });
      await addDoc(collection(db, 'contentAudit'), { action: 'comment:removed-from-report', articleSlug: report.targetId, actorId, createdAt: serverTimestamp() });
  ...
  return <section className="admin-section">
    ...
    {reports.filter((report) => report.status === 'open').length > 0 && <section className="admin-comments"><h3>Açık içerik raporları ({reports.filter((report) => report.status === 'open').length})</h3>{reports.filter((report) => report.status === 'open').map((report) => <article key={report.id}><div><strong>{report.targetType === 'comment' ? 'Yorum' : 'Konu'}: {reportTarget(report)}</strong><p>{report.reason}</p><small>Kaynak ID: {report.targetId} · {report.createdAt?.toLocaleDateString('tr-TR') || 'Yeni rapor'}</small></div><div><button className="admin-secondary compact" onClick={() => void resolveReport(report)}>Raporu kapat</button>{report.targetType === 'comment' && <button className="admin-danger compact" onClick={() => void removeReportedComment(report)}>Yorumu kaldır</button>}</div></article>)}</section>}
