...
export default function AdminOperationPanel({ db, actorId }: { db: Firestore | null; actorId: string }) {
  ...
  const visible = useMemo(() => operations.filter((operation) => filter === 'all' || operation.status === filter), [filter, operations]);
  const metrics = useMemo(() => ({ active: operations.filter((item) => !['completed', 'cancelled'].includes(item.status)).length, awaiting: operations.filter((item) => item.status === 'awaiting_payment').length, completed: operations.filter((item) => item.status === 'completed').length }), [operations]);
