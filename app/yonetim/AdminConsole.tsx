...
export default function AdminConsole({ articleCount, rateCount }: { articleCount: number; rateCount: number }) {
  ...
  return <main className="admin-shell"><div className="admin-frame">
    ...
    <nav className="admin-nav" aria-label="Yönetim bölümleri">{([['overview', 'Genel bakış'], ['members', 'Üyeler'], ['moderation', 'Yorumlar'], ['access', 'Yetkiler']] as const).map(([id, label]) => <button key={id} className={view === id ? 'is-active' : ''} onClick={() => setView(id)}>{label}</button>)}</nav>
