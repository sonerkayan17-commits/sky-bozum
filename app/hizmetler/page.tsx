              ...
              <article key={article.slug} className="group overflow-hidden rounded-[22px] border border-white/10 bg-gradient-to-br from-white/[0.055] to-white/[0.018] shadow-[0_18px_50px_rgba(0,0,0,.22)] transition hover:-translate-y-1 hover:border-amber-300/25">
                <Link href={`/bilgi-merkezi/${article.slug}`} className="block">
                  <div className="relative aspect-[16/8.5] overflow-hidden border-b border-white/8 bg-[radial-gradient(circle_at_20%_20%,rgba(251,191,36,.14),transparent_35%),linear-gradient(145deg,#111722,#0a0e15)]">
                    {article.cover ? (
                      <Image src={article.cover} alt={article.coverAlt ?? article.title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-500 group-hover:scale-[1.025]" />
                    ) : relatedService ? (
                      <div className="absolute inset-0 grid place-items-center">
                        <div className="relative h-24 w-40 rounded-2xl border border-white/10 bg-white/[0.96] shadow-[0_18px_45px_rgba(0,0,0,.3)]">
                          <Image src={relatedService.logo} alt={`${relatedService.shortName} logosu`} fill sizes="160px" className="object-contain p-5" />
                        </div>
                      </div>
                    ) : null}
                  </div>
                  <div className="p-5">
                    ...
                    <h3 className="mt-3 text-lg font-black leading-snug tracking-tight text-white">{article.title}</h3>
                    <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-400">{article.excerpt}</p>
                  </div>
                </Link>
                {relatedService ? (
