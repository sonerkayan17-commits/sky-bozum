...
function ArchiveArticleCard({ article }: { article: ArticleItem }) {
  ...
  return <Link href={`/bilgi-merkezi/${article.slug}`} aria-labelledby={titleId} className={`focus-ring article-archive-card group relative flex min-h-[188px] flex-col overflow-hidden rounded-[20px] border p-5 pl-6 shadow-[0_16px_46px_-38px_rgba(0,0,0,.9)] transition duration-300 hover:-translate-y-1 ${presentation.card}`}> 
    ...
    <div className="mt-4 flex min-w-0 flex-1 flex-col">
      <h3 id={titleId} className="line-clamp-2 text-[17px] font-black leading-[1.32] tracking-[-0.018em] text-white transition group-hover:text-slate-100">{article.title}</h3>
      <p className="mt-2.5 line-clamp-2 text-[13px] leading-[1.4rem] text-slate-400">{article.excerpt}</p>
      <div className="mt-auto flex items-center gap-2 pt-4 text-[10px] font-bold uppercase tracking-[.09em] text-slate-600"><span className="truncate">{article.category}</span><span aria-hidden="true" className="text-slate-800">•</span><span className="shrink-0 normal-case tracking-normal text-slate-500">{article.readTime}</span></div>
    </div>
  </Link>;
}
