...
export default function MemberOperations() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(client.auth, (nextUser) => {
      ...
      return onSnapshot(query(collection(client.db!, 'operations'), where('memberId', '==', nextUser.uid)), (snapshot) => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!db || !user || !memberName.trim() || !contact.trim()) return;
