...
export default function PublicMemberProfile({ memberId }: { memberId: string }) {
  const [user,setUser]=useState<User|null>(null), [name,setName]=useState('Sky Bozum üyesi'), [avatar,setAvatar]=useState('');
  const [likes,setLikes]=useState(0), [gifts,setGifts]=useState(0), [message,setMessage]=useState(''), [notice,setNotice]=useState('');
  useEffect(()=>{ const {auth,db}=getFirebaseClient(); if(!auth||!db)return; const stop=onAuthStateChanged(auth,setUser); Promise.all([getDoc(doc(db,'publicProfiles',memberId)),getDocs(query(collection(db,'profileLikes'),where('receiverId','==',memberId)))]).then(([profile,likeDocs])=>{if(profile.exists()){setName(String(profile.data().displayName||'Sky Bozum üyesi'));setAvatar(String(profile.data().avatar||''))}setLikes(likeDocs.size)}); return stop; },[memberId]);
