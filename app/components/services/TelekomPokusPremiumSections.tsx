...
export default function TelekomPokusPremiumSections({ mode }: { mode: BrandMode }) {
  ...
  return (
    <>
      ...
      <section className="content-shell pb-16"><div className={`overflow-hidden rounded-[28px] border ${tone.panel}`}><div className="grid gap-0 lg:grid-cols-[.82fr_1.18fr]"><div className="border-b border-white/8 p-7 sm:p-9 lg:border-b-0 lg:border-r"><p className={`text-xs font-extrabold uppercase tracking-[0.18em] ${tone.text}`}>İşlem öncesi kontrol</p><h2 className="mt-3 text-3xl font-black tracking-tight">Dört kritik noktayı netleştirin</h2><p className="mt-4 text-sm leading-7 text-slate-400">Bu kontroller tamamlanmadan ürün satın almak, kart bilgisini girmek veya kod paylaşmak gereksiz risk oluşturabilir.</p><Link href="/guven-merkezi" className={`focus-ring mt-6 inline-flex rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-extrabold text-slate-200 ${tone.border} ${tone.hoverText}`}>Güven merkezini açın</Link></div><div className="grid sm:grid-cols-2">{item.controls.map(([title, text], index) => <article key={title} className={`p-6 sm:p-7 ${index % 2 === 0 ? 'sm:border-r sm:border-white/8' : ''} ${index < 2 ? 'border-b border-white/8' : ''}`}><span className={`flex h-9 w-9 items-center justify-center rounded-xl border text-xs font-black ${tone.softBorder} ${tone.softBg} ${tone.text}`}>{index + 1}</span><h3 className="mt-4 text-base font-black">{title}</h3><p className="mt-2 text-sm leading-7 text-slate-400">{text}</p></article>)}</div></div></div></section>
