    ...
    <main className="min-h-screen bg-[#090b10] text-white">
        ...
        <div className="content-shell relative grid items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              ...
              <ServiceSupportLink serviceName={service.name} serviceSlug={service.slug} source="hero" label={isPurchaseGuide ? 'Satın alma rehberini sorun' : 'Güncel oran alın'} purchaseGuide={isPurchaseGuide} />
      ...
      <section className="content-shell grid items-start gap-10 pb-16 lg:grid-cols-[1fr_340px]">
        ...
        <aside className="space-y-4 lg:sticky lg:top-28 lg:self-start">
          ...
          <div className="premium-card p-6"><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-rose-400">İşlem öncesi güvenlik</p><h2 className="mt-3 text-xl font-black">{isPurchaseGuide ? 'Satın almadan önce ayrıntıları okuyun.' : 'Kodunuzu hemen göndermeyin.'}</h2><ul className="mt-4 space-y-3">{(isPurchaseGuide ? purchaseGuideSafetyChecks : safetyChecks).map((check) => <li key={check} className="flex gap-3 text-xs leading-6 text-slate-400"><span aria-hidden="true" className="text-emerald-400">✓</span><span>{check}</span></li>)}</ul><ServiceSupportLink serviceName={service.name} serviceSlug={service.slug} source="sidebar" label="WhatsApp ile bilgi alın" className="btn-primary focus-ring mt-5 w-full" purchaseGuide={isPurchaseGuide} /></div>
