...
export function CodeCountCalculator() {
  ...
  let remaining = error ? 0 : numericTotal;
  const distribution = denominations.map(value=>{const count=Math.floor(remaining/value); remaining-=count*value; return {value,count};});
  return <div className="premium-card p-6 sm:p-8"><div className="grid gap-5 sm:grid-cols-2"><label htmlFor="code-total" className="text-xs font-bold text-slate-300">Toplam bakiye<input id="code-total" value={total} onChange={(event) => setTotal(event.target.value)} inputMode="decimal" maxLength={15} className="field mt-2" aria-invalid={Boolean(error)} aria-describedby="code-count-help" /></label><label htmlFor="code-values" className="text-xs font-bold text-slate-300">Kullanılabilen kod değerleri<input id="code-values" value={preset} onChange={(event) => setPreset(event.target.value)} inputMode="text" className="field mt-2" placeholder="1000,500,250,100" aria-invalid={Boolean(error && denominations.length === 0)} aria-describedby="code-count-help" /></label></div><FieldMessage id="code-count-help" error={error}>Araç, büyük değerden başlayarak tam kod sayısını ve artan bakiyeyi hesaplar.</FieldMessage><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" role="status" aria-live="polite" aria-atomic="true">{distribution.map(row=><div key={row.value} className="rounded-2xl border border-white/10 bg-white/[.03] p-5"><p className="text-xs font-bold text-slate-400">{money(row.value)} TL değerinde</p><strong className="mt-2 block text-3xl font-black text-amber-300">{row.count} kod</strong></div>)}</div><div className="mt-3 rounded-2xl border border-white/10 bg-white/[.025] p-4 text-sm text-slate-400">Tam koda ayrılamayan bakiye: <strong className="text-white">{money(remaining)} TL</strong></div><p className="mt-4 text-xs leading-6 text-slate-500">Bu dağılım yalnızca matematiksel planlama içindir. Satın alınabilecek kod değerleri marka, bölge ve satış kanalına göre farklı olabilir.</p></div>;
}
...
const wizardData = {
  ...
  'gift-card': { title:'Dijital kod / gift card', text:'Markayı, kod değerini, para birimini ve mağaza bölgesini netleştirerek başlayın.', tool:'/araclar/gift-card-hesaplama', guide:'/bilgi-merkezi/kategori/hediye-kartlari', caution:'Bölgesi belirsiz veya daha önce kullanılmış kod satın almayın.' },
  'cuzdan': { title:'Dijital cüzdan bakiyesi', text:'Paycell veya Pokus bakiyeniz varsa ana hesaplayıcıdan hizmeti seçin; cüzdan bakiyesi ile mobil ödeme limitini karıştırmayın.', tool:'/araclar#oran-hesapla', guide:'/bilgi-merkezi/kategori/dijital-cuzdanlar', caution:'Kullanılabilir bakiye ve günlük işlem limitini uygulama içinden kontrol edin.' },
  'cihaz': { title:'Faturaya ek cihaz', text:'Peşin fiyat, peşinat, vade ve aylık ek bedeli ayrı ayrı girerek toplam maliyeti görün.', tool:'/araclar/faturaya-ek-cihaz-hesaplama', guide:'/bilgi-merkezi/kategori/faturaya-ek-cihaz', caution:'Son kararı operatörün sözleşme ve ödeme planını gördükten sonra verin.' },
