...
function collectRenderableSource(entryFile, visited = new Set(), depth = 0) {
  if (depth > 8 || visited.has(entryFile)) return '';
  visited.add(entryFile);
  const content = fs.readFileSync(entryFile, 'utf8');
  let merged = content;
  const importPattern = /(?:import|export)\s+(?:[^'";]+?\s+from\s+)?['"]([^'"]+)['"]/g;
  for (const match of content.matchAll(importPattern)) {
    const resolved = resolveLocalImport(entryFile, match[1]);
    if (resolved) merged += `\n${collectRenderableSource(resolved, visited, depth + 1)}`;
  }
  return merged;
}
...
for (const file of requiredFiles) {
  if (exists(file)) pass(`${file} mevcut`);
  else fail(`${file} eksik`);
}
...
for (const key of ['NEXT_PUBLIC_SITE_URL', 'NEXT_PUBLIC_WHATSAPP_URL']) {
  if (env.includes(key)) pass(`.env.example içinde ${key} tanımlı`);
  else fail(`.env.example içinde ${key} eksik`);
}
