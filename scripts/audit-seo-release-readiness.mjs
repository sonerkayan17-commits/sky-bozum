...
const identity = read('app/lib/siteIdentity.ts');
...
if (!identity.includes('NEXT_PUBLIC_SITE_URL') || !identity.includes('EXPECTED_PRODUCTION_ORIGIN') || !seo.includes('PRODUCTION_SITE_URL') || !seo.includes('ALLOW_INDEXING')) findings.push('SEO URL/indexing must be env-based and production-domain gated.');
