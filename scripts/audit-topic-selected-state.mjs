...
const quickStart = /onClick=\{\(\) => openTopic\(topicItem\)\}[^>]*aria-pressed=\{false\}[^>]*className="focus-ring group (?:relative overflow-hidden )?rounded-2xl/.test(source);
...
if (!quickStart || !emptyState) {
  console.error('Konu seçim düğmelerinin erişilebilir seçili durumları eksik.');
  process.exit(1);
}
